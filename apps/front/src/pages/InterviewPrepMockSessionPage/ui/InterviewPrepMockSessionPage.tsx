...
export function InterviewPrepMockSessionPage() {
    ...
    if (isFinished) {
      return (
        <div className="flex flex-col items-center justify-center gap-4 py-4 text-center">
          ...
          <h2 className="text-lg font-bold text-[#f8fafc]">Интервью завершено</h2>
          <p className="text-sm text-[#94a3b8]">{sessionClosing || 'Все этапы пройдены.'}</p>
      ...
      return (
        <>
          ...
          <div className="mb-3 flex items-center gap-1.5">
            ...
            <span className="text-xs font-semibold uppercase tracking-wide text-[#fbbf24]">Вопрос от интервьюера</span>
          </div>
          <p className="text-sm font-medium leading-relaxed text-[#f8fafc]">{currentQuestion.prompt}</p>
          <p className="mt-3 text-xs leading-relaxed text-[#64748b]">
            Ответьте в поле {isMobile ? 'ниже' : 'справа'}. После ответа AI оценит вашу реакцию.
          </p>
        </>
      )
    }
    ...
    if (reviewLoading) {
      return (
        <div className="flex flex-col items-center justify-center gap-3 py-6">
          <Spinner size="md" />
          <p className="text-sm text-[#94a3b8]">Анализирую ответ...</p>
        </div>
      )
    }
  ...
  const TextAnswerArea = ({ placeholder }: { placeholder: string }) => (
    <div className="flex-1 flex flex-col p-4 bg-[#0f172a]">
      <label className="text-xs font-semibold text-[#94a3b8] mb-2 block">Ваш ответ</label>
      <textarea
        value={textAnswer}
      ...
      <div className="flex min-h-screen flex-col bg-[#0f172a]">
        <header className="border-b border-[#1e293b] bg-[#0f172a] px-4 pt-3 pb-4">
          <div className="flex items-start gap-3">
            <button onClick={() => navigate('/growth/interview-prep')} className="mt-0.5 flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-2xl bg-[#1e293b] text-[#94a3b8]">
              <ArrowLeft className="w-4 h-4" />
            </button>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-bold text-[#f8fafc]">Mock · {blueprintTitle || companyTag || 'General'}</p>
            ...
            {isFinished ? (
              <span className="rounded-full bg-[#1e293b] px-2 py-1 text-[11px] font-medium text-[#64748b]">Завершено</span>
            ) : (
        ...
        <div className="flex flex-1 flex-col gap-4 px-4 pt-4 pb-24">
          ...
          {isFinished ? (
            ...
            <div className="rounded-[24px] border border-[#1e293b] bg-[#1e293b] p-4">
              <label className="mb-2 block text-xs font-semibold text-[#94a3b8]">Ваш ответ</label>
              <textarea
                value={textAnswer}
                onChange={e => setTextAnswer(e.target.value)}
          ...
          <div className="rounded-[24px] border border-[#1e293b] bg-[#1e293b] p-4">
            <div className="mb-3 flex items-center gap-2">
              ...
              <span className="text-sm font-bold text-[#f8fafc]">AI Обратная связь</span>
            </div>
            <AIFeedbackContent />
          </div>
        </div>
    ...
    <div className="flex flex-col h-screen bg-[#0f172a] overflow-hidden">
      <header className="h-[52px] bg-[#0f172a] border-b border-[#1e293b] flex items-center justify-between px-5 flex-shrink-0">
        <div className="flex items-center gap-3">
          <button onClick={() => navigate('/growth/interview-prep')} className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-[#1e293b] text-[#94a3b8] transition-colors">
            <ArrowLeft className="w-4 h-4" />
          </button>
          <div>
            <p className="text-sm font-bold text-[#f8fafc]">Mock Interview · {blueprintTitle || companyTag || 'General'}</p>
          ...
          {isFinished && (
            <span className="px-2 py-0.5 rounded-full bg-[#1e293b] text-[#64748b] text-[11px] font-medium">Завершено</span>
          )}
        </div>
        ...
        <div className="flex-1 flex flex-col min-w-0">
          {isFinished ? (
            <div className="flex-1 flex items-center justify-center bg-[#0f172a]">
              <p className="text-sm text-[#475569]">Все этапы завершены</p>
            </div>
          ) : isInQuestionsPhase ? (
            <TextAnswerArea placeholder="Напишите ваш ответ на вопрос..." />
          ) : CODE_KINDS.has(stageKind) ? (
            <>
              <EditorBar />
            ...
            <TextAnswerArea placeholder="Напишите ваш ответ..." />
          )}
        </div>
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-bold text-[#f8fafc]">{passed ? 'Тесты прошли' : 'Тесты не прошли'}</p>
          {totalCount > 0 && (
            <p className="text-xs text-[#64748b]">{passedCount} из {totalCount} тестов</p>
          )}
        </div>
      </div>
      ...
      {lastError && (
        <div className="bg-[#450a0a] border border-[#7f1d1d] rounded-lg p-3">
          <p className="text-xs font-semibold text-[#fca5a5] mb-1">Ошибка</p>
...
function ReviewPanel({ review, stageKind }: { review: any; stageKind: string }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {score !== null && (
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[#64748b]">из 10</p>
          </div>
        </div>
      )}
      ...
      {summary && (
        <div>
          <p className="text-xs font-semibold text-[#94a3b8] mb-1.5">Резюме</p>
      ...
      {strengths.length > 0 && (
        <div>
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <p className="text-xs font-semibold text-[#4ade80]">Сильные стороны</p>
          </div>
      ...
      {gaps.length > 0 && (
        <div>
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <p className="text-xs font-semibold text-[#fbbf24]">Пробелы</p>
          </div>
          <ul className="flex flex-col gap-1">
      ...
      {stageKind === 'system_design' && missingTopics.length > 0 && (
        <div>
          <div className="flex items-center gap-1.5 mb-1.5">
            ...
            <p className="text-xs font-semibold text-[#f87171]">Упущено</p>
          </div>
          <ul className="flex flex-col gap-1">
      ...
      {followUpQuestions.length > 0 && (
        <div>
          <p className="text-xs font-semibold text-[#94a3b8] mb-1.5">Дополнительные вопросы</p>
          <ul className="flex flex-col gap-1">
