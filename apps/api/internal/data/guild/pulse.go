...
func (r *Repo) GetGuildPulse(ctx context.Context, guildID uuid.UUID) (*model.GuildPulse, error) {
	...
	if err := r.data.DB.QueryRow(ctx,
		`SELECT member_count FROM guilds WHERE id = $1`, guildID,
	).Scan(&totalMembers); err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, kratoserrors.NotFound("GUILD_NOT_FOUND", "guild not found")
		}
		return nil, fmt.Errorf("get guild member count: %w", err)
	}

	pulse := &model.GuildPulse{
		TotalMembers:  totalMembers,
		WeekActivity:  make([]*model.GuildDayActivity, 0, 7),
		RecentActions: make([]*model.GuildMemberAction, 0, 20),
	}
...
GROUP BY days.d
ORDER BY days.d ASC`, guildID, weekAgo, todayStart)
	if err != nil {
		return nil, fmt.Errorf("query week activity: %w", err)
	}
	defer rows.Close()
	for rows.Next() {
		var da model.GuildDayActivity
...
LIMIT 20`, guildID, weekAgo)
	if err != nil {
		return nil, fmt.Errorf("query recent actions: %w", err)
	}
	defer actionRows.Close()
	for actionRows.Next() {
		var a model.GuildMemberAction
...
func (r *Repo) GetGuildMemberStats(ctx context.Context, guildID uuid.UUID) ([]*model.GuildMemberStats, error) {
	rows, err := r.data.DB.Query(ctx, `
SELECT
  cm.user_id,
  COALESCE(NULLIF(u.first_name,''),'') AS first_name,
...
WHERE cm.guild_id = $1
ORDER BY cm.joined_at ASC`, guildID)
	if err != nil {
		return nil, fmt.Errorf("query member stats: %w", err)
	}
	defer rows.Close()

	stats := make([]*model.GuildMemberStats, 0, 12)
	for rows.Next() {
		var s model.GuildMemberStats
		if err := rows.Scan(
...
func (r *Repo) CreateGuildChallenge(ctx context.Context, req model.CreateGuildChallengeRequest) (*model.GuildChallenge, error) {
	id := uuid.New()
	if _, err := r.data.DB.Exec(ctx, `
INSERT INTO guild_challenges (id, guild_id, template_key, target_value, starts_at, ends_at, created_by)
VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		id, req.GuildID, req.TemplateKey, req.TargetValue, req.StartsAt, req.EndsAt, req.CreatedBy,
	); err != nil {
		return nil, fmt.Errorf("insert guild challenge: %w", err)
	}
...
func (r *Repo) GetActiveGuildChallenge(ctx context.Context, guildID uuid.UUID) (*model.GuildChallenge, error) {
	var ch model.GuildChallenge
	err := r.data.DB.QueryRow(ctx, `
SELECT id, guild_id, template_key, target_value, starts_at, ends_at, created_by, created_at
FROM guild_challenges
WHERE guild_id = $1 AND ends_at > now() AND starts_at <= now()
ORDER BY created_at DESC
LIMIT 1`, guildID).Scan(
		&ch.ID, &ch.GuildID, &ch.TemplateKey, &ch.TargetValue,
...
func (r *Repo) computeChallengeProgress(ctx context.Context, ch *model.GuildChallenge) ([]*model.ChallengeMemberProgress, error) {
	var query string
