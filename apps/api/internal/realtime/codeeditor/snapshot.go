...
func (h *Hub) loadRoomSnapshot(roomID string) {
...
func (h *Hub) flushSnapshot(roomID, code, language string) {
	parsedRoomID := mustParseUUID(roomID)
	if parsedRoomID == uuid.Nil {
		return
	}
...
func (h *Hub) flushActorSnapshot(client *codeEditorClient, state *codeEditorEditorState) {
...
func (h *Hub) flushActorSnapshotByState(item pendingActorSnapshot) {
