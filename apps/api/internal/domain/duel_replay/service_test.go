...
func TestGetReplay_AllowsParticipant(t *testing.T) {
	...
	viewer := p1
	result, err := svc.GetReplay(context.Background(), repo.summary.ID, &viewer)
	if err != nil {
		t.Fatal(err)
	}
...
func TestGetReplay_RejectsOutsider(t *testing.T) {
	...
	svc := NewService(Config{Repository: repo})
	outsider := uuid.New()
	_, err := svc.GetReplay(context.Background(), repo.summary.ID, &outsider)
...
func TestGetReplay_NotFound(t *testing.T) {
	...
	svc := NewService(Config{Repository: repo})
	_, err := svc.GetReplay(context.Background(), uuid.New(), nil)
...
func TestGetReplay_AdminBypass(t *testing.T) {
	...
	svc := NewService(Config{Repository: repo})
	_, err := svc.GetReplay(context.Background(), repo.summary.ID, nil)
	if err != nil {
...
func TestRecordEvent_ValidatesTimeRange(t *testing.T) {
	...
	_, err := svc.RecordEvent(context.Background(), model.RecordEventInput{
...
func TestRecordEvent_HappyPath(t *testing.T) {
	...
	svc := NewService(Config{Repository: repo})
	ev, err := svc.RecordEvent(context.Background(), model.RecordEventInput{
