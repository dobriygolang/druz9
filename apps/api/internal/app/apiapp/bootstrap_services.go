...
import (
	...
	interviewliveservice "api/internal/api/interview_live"
...
type serviceContext struct {
	...
	interviewLiveService   *interviewliveservice.Implementation
	premiumService         *premiumapi.Implementation
}
...
func initializeServices(bootstrap *bootstrapContext, storage *storageContext) (*serviceContext, error) {
	...
	return &serviceContext{
		...
		hubService:   hubservice.New(storage.profileRepo, missionServiceDomain, eventServiceDomain, arenaServiceDomain, guildServiceDomain, seasonPassDomain).WithRegionStatsRepository(hubdata.NewRegionStatsRepo(storage.store)),
		...
		interviewLiveService: interviewliveservice.New(aiReviewService, storage.interviewLiveRepo, storage.premiumRepo).
			WithMentorResolver(mentorResolverAdapter{repo: storage.aiMentorRepo}),
		premiumService: newPremiumService(bootstrap, storage),
	}, nil
}

func newPremiumService(bootstrap *bootstrapContext, storage *storageContext) *premiumapi.Implementation {
