...
func TestCreateTask(t *testing.T) {
	...
	t.Run("creates task and returns response", func(t *testing.T) {
		t.Parallel()
		...
		req := &v1.CreateTaskRequest{
			...
			Difficulty:  v1.TaskDifficulty_TASK_DIFFICULTY_MEDIUM,
			TaskType:    v1.TaskType_TASK_TYPE_ALGORITHM,
			StarterCode: "function solve() {}",
		}
		expectedTask := &codeeditordomain.Task{
			ID:         taskID,
			Title:      req.Title,
			Statement:  req.Statement,
			Difficulty: codeeditordomain.TaskDifficultyMedium,
		}

		mockService := mocks.NewService(t)
		mockService.On("CreateTask", mock.Anything, mock.Anything).Return(expectedTask, nil).Once()

		mockRealtime := mocks.NewRealtimePublisher(t)

		impl := New(mockService, mockRealtime)

		resp, err := impl.CreateTask(context.Background(), req)
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
		if resp == nil {
...
func TestUpdateTask(t *testing.T) {
	...
	t.Run("updates task and returns response", func(t *testing.T) {
		t.Parallel()
		...
		req := &v1.UpdateTaskRequest{
			...
			Difficulty: v1.TaskDifficulty_TASK_DIFFICULTY_HARD,
		}
		expectedTask := &codeeditordomain.Task{
			ID:         taskID,
			Title:      req.Title,
			Statement:  req.Statement,
			Difficulty: codeeditordomain.TaskDifficultyHard,
		}

		mockService := mocks.NewService(t)
		mockService.On("UpdateTask", mock.Anything, mock.Anything).Return(expectedTask, nil).Once()

		mockRealtime := mocks.NewRealtimePublisher(t)

		impl := New(mockService, mockRealtime)

		resp, err := impl.UpdateTask(context.Background(), req)
		if err != nil {
			t.Errorf("unexpected error: %v", err)
		}
		if resp == nil {
	...
	t.Run("returns error for invalid task id", func(t *testing.T) {
		...
		_, err := impl.UpdateTask(context.Background(), &v1.UpdateTaskRequest{TaskId: "invalid-uuid"})
		if err == nil {
