...
var trainingNodes = []nodeDefinition{
	...
	{ID: "strings", X: 420, Y: 640, Label: "strings", Branch: "core", InitiallyUnlocked: true, Topics: []string{"string", "strings"}, Prereq: []string{"arrays"}, Unlocks: []string{"sorting", "two-ptr"}, Title: "Core · Strings", Projects: 2, Hours: "4–6h", Hints: []string{"Pay attention to rune vs byte boundaries.", "Look for prefix/suffix invariants before brute force."}},
	{ID: "hashmap", X: 660, Y: 640, Label: "hash\nmap", Branch: "core", InitiallyUnlocked: true, Topics: []string{"hashmap", "hash_map", "map", "maps"}, Prereq: []string{"arrays"}, Unlocks: []string{"sliding"}, Title: "Core · Hash maps", Projects: 2, Hours: "4–6h", Hints: []string{"Reach for a map when direct indexing is unstable.", "Store only the minimum signal you need for the invariant."}},
	{ID: "sorting", X: 300, Y: 580, Label: "sorting", Branch: "core", InitiallyUnlocked: true, Topics: []string{"sort", "sorting"}, Prereq: []string{"strings"}, Unlocks: []string{"tree-basics"}, Title: "Core · Sorting", Projects: 2, Hours: "4–6h", Hints: []string{"Sort first if it simplifies the invariant.", "After sorting, re-check whether two pointers collapse the search space."}},
	{ID: "two-ptr", X: 540, Y: 580, Label: "2-ptr", Branch: "core", InitiallyUnlocked: true, Topics: []string{"two-ptr", "two_ptr", "two-pointers", "two_pointer"}, Prereq: []string{"arrays", "strings"}, Unlocks: []string{"sliding", "graph-basics"}, Title: "Core · Two pointers", Projects: 2, Hours: "4–6h", Hints: []string{"Move only one pointer per invariant break.", "Let the sorted order or window invariant justify each move."}},
	{ID: "sliding", X: 780, Y: 580, Label: "sliding\nwindow", Branch: "core", InitiallyUnlocked: true, Topics: []string{"sliding", "sliding-window", "sliding_window"}, Prereq: []string{"hashmap", "two-ptr"}, Unlocks: []string{"dp-basics"}, Title: "Core · Sliding window", Projects: 2, Hours: "4–6h", Hints: []string{"Define the exact condition that makes the window valid.", "Only shrink once the window has already captured the target state."}},
	{ID: "tree-basics", X: 200, Y: 480, Label: "trees\nbasics", Branch: "tree", InitiallyUnlocked: true, Topics: []string{"tree", "trees", "binary-tree", "binary_tree"}, Prereq: []string{"sorting"}, Unlocks: []string{"bst", "heap"}, Title: "Trees · Basics", Projects: 2, Hours: "4–6h", Hints: []string{"Choose DFS when the state is local to a path.", "Write the base case before thinking about the recursive step."}},
	{ID: "bst", X: 120, Y: 400, Label: "BST", Branch: "tree", InitiallyUnlocked: true, Topics: []string{"bst", "binary-search-tree", "binary_search_tree"}, Prereq: []string{"tree-basics"}, Unlocks: []string{"segment", "tree-dp"}, Title: "Trees · Binary search trees", Projects: 2, Hours: "4–6h", Hints: []string{"Exploit ordering instead of scanning both branches.", "Write down the invariant for values allowed in each subtree."}},
	{ID: "heap", X: 240, Y: 380, Label: "heap", Branch: "tree", InitiallyUnlocked: true, Topics: []string{"heap", "priority-queue", "priority_queue"}, Prereq: []string{"tree-basics"}, Unlocks: []string{"lca"}, Title: "Trees · Heap", Projects: 2, Hours: "4–6h", Hints: []string{"Use the heap to keep only the next best candidate.", "Push and pop around the same invariant, not around the whole dataset."}},
	...
	{ID: "graph-basics", X: 460, Y: 480, Label: "graph\nbasics", Branch: "graph", InitiallyUnlocked: true, Topics: []string{"graph", "graphs"}, Prereq: []string{"two-ptr"}, Unlocks: []string{"graph-bfs", "graph-dfs"}, Title: "Graphs · Basics", Projects: 2, Hours: "4–6h", Hints: []string{"Start by deciding the right graph representation.", "Reachability usually means adjacency list plus visited structure."}},
	{ID: "graph-bfs", X: 380, Y: 400, Label: "BFS", Branch: "graph", InitiallyUnlocked: true, Topics: []string{"bfs", "graph-bfs", "graph_bfs"}, Prereq: []string{"graph-basics"}, Unlocks: []string{"dijkstra"}, Title: "Graphs · Breadth-first search", Projects: 2, Hours: "4–6h", Hints: []string{"Use BFS when distance in edges matters.", "Push to the queue exactly once per node."}},
	{ID: "graph-dfs", X: 540, Y: 400, Label: "DFS", Branch: "graph", InitiallyUnlocked: true, Topics: []string{"dfs", "graph-dfs", "graph_dfs"}, Prereq: []string{"graph-basics"}, Unlocks: []string{"dijkstra", "union-find"}, Title: "Graphs · Depth-first search", SpecialDesc: "DFS, topological sort, cycle detection, connected components. Build speed and graph intuition before shortest paths.", Projects: 3, Hours: "6–8h", Rewards: []string{"+320 ✦", "+60 gold", "Graph Walker badge"}, Hints: []string{"Start with an adjacency list from the edge list.", "Each fresh DFS should consume exactly one connected component.", "Increment the answer when you launch DFS from an unvisited node."}},
	...
	{ID: "dp-basics", X: 740, Y: 480, Label: "DP\nbasics", Branch: "dp", InitiallyUnlocked: true, Topics: []string{"dp", "dynamic-programming", "dynamic_programming"}, Prereq: []string{"sliding"}, Unlocks: []string{"knapsack", "dp-strings"}, Title: "DP · Basics", Projects: 2, Hours: "4–6h", Hints: []string{"Write the state definition before the transition.", "Memoized DFS and bottom-up DP are the same recurrence in different clothes."}},
