...
func StartBusinessMetrics(
	...
	refresh := func() {
		...
		roomsCount, err := codeEditorRepo.CountOpenRooms(refreshCtx, time.Now().Add(-codeRoomIdleTTL))
		if err != nil {
		...
		matchesCount, err := arenaRepo.CountOpenMatches(refreshCtx, time.Now().Add(-arenaIdleTTL))
		if err != nil {
		...
		usersCount, err := profileRepo.CountActiveUsers(refreshCtx, time.Now().Add(-activeUsersWindow))
		if err != nil {
