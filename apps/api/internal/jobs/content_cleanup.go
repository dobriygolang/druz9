...
func StartContentCleanup(_ klog.Logger, eventRepo *eventdata.Repo, podcastRepo *podcastdata.Repo) func() error {
	...
	runCleanup := func() {
		...
		if eventRepo != nil {
			deleted, err := eventRepo.CleanupExpiredEvents(cleanupCtx, expiredEventsTTL)
			if err != nil {
		...
		if podcastRepo != nil {
			deleted, err := podcastRepo.CleanupStaleDrafts(cleanupCtx, stalePodcastDraftTTL)
			if err != nil {
